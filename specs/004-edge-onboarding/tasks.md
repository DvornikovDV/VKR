...
- [X] T029 [US3] Update shared client edge contracts to expose lifecycle, masked onboarding metadata, and telemetry-ready filtering semantics for downstream `002-frontend` and `003-dashboard` consumers in `client/src/shared/api/edgeServers.ts` and `client/src/shared/hooks/useEdgeStatus.ts`
...
- [X] T030 [P] Update shipped contract documents to match implementation details in `specs/004-edge-onboarding/contracts/openapi.yaml`, `specs/004-edge-onboarding/contracts/edge-socket-contract.md`, and `specs/004-edge-onboarding/contracts/lifecycle-state-machine.md`
- [X] T031 [P] Run quickstart validation and OpenAPI lint from `specs/004-edge-onboarding/quickstart.md` and `specs/004-edge-onboarding/contracts/openapi.yaml`
