        ...
        {!savedDiagram ? (
          ...
          <ul className="mt-2 space-y-2">
            {runtimeWidgets.map((widget) => (
              <li
                key={widget.widgetId}
                className="rounded-md border border-[var(--color-surface-border)] bg-[var(--color-surface-200)] px-3 py-2 text-sm text-white"
              >
                ...
                {!widget.isSupported ? (
                  ...
                  <p className="mt-1 text-xs text-[#94a3b8]">Value: {formatRuntimeValue(widget.value)}</p>
                )}
              </li>
            ))}
          </ul>
