...
describe('repro_task_T056', () => {
  it('renders canonical lifecycle and availability rows for assigned blocked edges on /hub/edge', async () => {
    ...
    expect(card.getByText('Состояние')).toBeInTheDocument()
    expect(card.getAllByText('Заблокирован')).toHaveLength(2)
    expect(card.getByText('Доступность')).toBeInTheDocument()
    expect(card.getAllByText('Не в сети')).toHaveLength(2)
    expect(card.getByText('Последний сигнал')).toBeInTheDocument()
