...
describe('repro_task_T021', () => {
  it('prepares onboarding handshake and persists activation credential for reconnect', async () => {
    ...
    const bootstrap = await (edgeTransport as Record<string, (...args: unknown[]) => Promise<unknown>>)
      .prepareEdgeRuntimeHandshake({
        store,
        edgeId: 'edge-t021',
    ...
    expect((bootstrap as Record<string, unknown>).credentialSource).toBe('onboarding_env')
    expect((bootstrap as Record<string, unknown>).handshakeAuth).toMatchObject({
      edgeId: 'edge-t021',
      credentialMode: 'onboarding',
      credentialSecret: 'onboarding-secret-t021',
    })

    const persisted = await (edgeTransport as Record<string, (...args: unknown[]) => Promise<unknown>>)
      .applyEdgeActivationEvent({
        store,
    ...
    expect((persisted as Record<string, unknown>).credentialMode).toBe('persistent')
    expect((persisted as Record<string, unknown>).version).toBe(3)

    const reconnect = await (edgeTransport as Record<string, (...args: unknown[]) => Promise<unknown>>)
      .prepareEdgeRuntimeHandshake({
        store,
      })
    expect((reconnect as Record<string, unknown>).credentialSource).toBe('persisted')
    expect((reconnect as Record<string, unknown>).handshakeAuth).toMatchObject({
      edgeId: 'edge-t021',
