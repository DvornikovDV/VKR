# Налая разработки: Привязки + Сохранение/Лоад

**Версия**: 1.0  
**Помечание**: Для контекста нейросети (не для разработчика)

---

## Отимальные итнерарии

```
Неделя 1:
  Mon ← Фаза 1: Проверка
  Tue-Wed ← Фаза 2 и Фазы A-B-D ПАРАЛЛЕЛНО
  Thu ← Фаза 3-4
  Fri ← Фаза 5 и Фазы E-F ПАРАЛЛЕЛНО
```

---

## ПОГОВОР: ПАРАЛЛЕЛНУЕ ОСИ

### ОСИ  1: Сохранение/Лоад (файлы schema + bindings)
...
- availableDevices = только тэги этой машины
- Валидация: машина выбрана? тэг из нее?
...
- Проверка UI кнопок
- Тестирование текущей ф-ц

---

## ПАРАЛЛЕЛМОЕ: УТД 1 + УТД 2

### УТД 1: ОСИ 1 (Сохранение/Лоад)
...
- Валидация уровень 2: тэг из быбыбыбыбыбыэ машины?
- Проверка в assignDeviceToElement()

---

## ФИНАЛЬНЫЕ: ОБЕ ОСИ КОНЧАУТСЙ
...
- Если мисматч: confirm + переключение

**НАЦОНАЛОЭ**: Тестирование (вять из Фазы 5 и Фазы F)
- Сценарий 2: Валидация
  - Сохранение bindings без машины: ошибка?
  - Употребление тэга другой машины: ошибка?
  - Гружка несовместимых bindings: confirm + свитч?
...
## МИННОНУМ: МНОГОРИчНАЯ ПАЛІТ

БВО УПТИ = 4 координирсяются + 2 выполняются параллельно

Матрица исполнителя:

| Day | UDI 1 (Сох/Лоад) | UDI 2 (Привязки) |
|-----|---------------------------|---------------------------|
| Mon | Фаза 1 (ОБГ) | - |
...
**ОСНОВНОЕ**: Автозапоминание (currentSchemaId) работает в ДЭЛЕНО два утд.
